...
`KeyboardExtended` is an extension of the `Microsoft.Xna.Framework.Input.Keyboard` class. Unlike the default MonoGame implementation, `KeyboardExtended` will internally track both the keyboard state of the previous frame and the keyboard state of the current frame. This allows for easy comparisons between the previous and current state to check for single frame key presses.
