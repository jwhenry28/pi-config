...
| `writePrompt(cwd, name, content)` | `.pi/prompts/<name>.md` |
...
`createComponentTest()` always creates its own temp dirs automatically. There is no `cwd` option — the harness manages the temp dir lifecycle internally, cleaning them up in `dispose()`.

- **`test.cwd`** — isolated data directory for project-local files (.pi/prompts/, .pi/workflows/, etc.)
- **`test.homeDir`** — isolated home directory for global files (~/.pi/plugins/, ~/.pi/prompts/, etc.)

These are deliberately separate to catch code that incorrectly reads from cwd instead of home (or vice versa).

Use `test.cwd` for file assertions on project-local data:
...
test = await createComponentTest({
  ...
  initialPlugins: [{ name: "my-repo", skills: [{ name: "cool-skill", content: "..." }], workflows: [{ name: "deploy", config: {...} }], files: [{ path: "README.md", content: "# test" }] }],
  initialWorkflows: [{ name: "my-flow", config: { name: "my-flow", steps: [] } }],
  initialTodos: [{ filepath: "todo.md", items: [{ text: "Task A", checked: false }] }],
  initialPrompts: [{ name: "my-prompt", content: "Prompt content" }],
  initialHomePrompts: [{ name: "pack/task", content: "Home prompt content" }],
});
```
