...
describe("workflow extension (component)", () => {
  ...
  describe("workflow prompt memory", () => {
    it("stores the initial workflow prompt under workflow-prompt on start", async () => {
      ...
      expect(readKey(test.cwd, workflowId, "workflow-prompt")).toBe(
        "Original canonical prompt",
      );
    }, 15000);
  ...
  describe("prompt memory between steps", () => {
    it("uses updated workflow-prompt memory value for the next step", async () => {
      ...
      test.sendUserMessage(
        "/workflow prompt-revision Build X using the old approach",
      );
      await test.mockAgentResponse({
        text: "We discovered the better approach.",
      });
      await test.waitForIdle();
      ...
      writeKey(
        ...
        workflowId,
        "workflow-prompt",
        "Build X using the revised approach",
      );

      test.sendUserMessage("/workflow continue");
      await new Promise((r) => setTimeout(r, 300));
      await test.mockAgentResponse({
        text: "Implemented the revised approach.",
      });
      await test.waitForIdle();
  ...
  describe("fresh context per step", () => {
    it("step 2 does not see step 1 agent response in LLM context", async () => {
      ...
      const step1UserMsg = capturedContexts[0].messages.find(
        (m: any) => m.role === "user",
      );
      if (step1UserMsg) {
...
`,
      ...
      expect(skillMessages[0].content).toContain(
        "FAKE_SKILL_CONTENT_FOR_TESTING",
      );
    }, 15000);
  });

  describe("command conditions", () => {
    it("injects condition explanation after slash-command condition jump", async () => {
      test = await createComponentTest();
      registerMockModel(test);

      writeTodo(test.cwd, "explanation-todo.md", [
        { text: "Task A", checked: false },
      ]);

      writeWorkflow(test.cwd, "condition-explanation", {
        name: "condition-explanation",
        steps: [
          {
            name: "DoWork",
            model: "mock-model",
            prompt: "Work on todos",
            conditions: [
              {
                command: "incomplete-todos-remaining",
                args: { todoFilepath: "explanation-todo.md" },
                jump: "Review",
                explanation: "The TODO list still has unchecked items.",
              },
            ],
          },
          { name: "Review", model: "mock-model", prompt: "Review the result" },
        ],
      });
      ...
      test.sendUserMessage("/workflow condition-explanation Fix todos");
      await test.mockAgentResponse({ text: "Initial work" });
      await new Promise((r) => setTimeout(r, 200));
      await test.mockAgentResponse({ text: "Review complete" });
      await test.waitForIdle();

      expect(capturedContexts.length).toBeGreaterThanOrEqual(2);
      const reviewPromptText = JSON.stringify(capturedContexts[1].messages);
      expect(reviewPromptText).toContain("You previously executed step DoWork");
      expect(reviewPromptText).toContain(
        "The reason for this move is: The TODO list still has unchecked items.",
      );
      expect(reviewPromptText).not.toContain("1 of 1 tasks remaining");
    }, 15000);

    it("omits condition explanation paragraph when jumping condition has no explanation", async () => {
      test = await createComponentTest();
      registerMockModel(test);

      writeTodo(test.cwd, "no-explanation-todo.md", [
        { text: "Task A", checked: false },
      ]);

      writeWorkflow(test.cwd, "condition-no-explanation", {
        name: "condition-no-explanation",
        steps: [
          {
            name: "DoWork",
            model: "mock-model",
            ...
            conditions: [
              {
                command: "incomplete-todos-remaining",
                args: { todoFilepath: "no-explanation-todo.md" },
                jump: "Review",
              },
            ],
          },
      ...
      test.sendUserMessage("/workflow condition-no-explanation Fix todos");
      await test.mockAgentResponse({ text: "Initial work" });
      await new Promise((r) => setTimeout(r, 200));
      await test.mockAgentResponse({ text: "Review complete" });
      await test.waitForIdle();
      ...
      expect(capturedContexts.length).toBeGreaterThanOrEqual(2);
      const reviewPromptText = JSON.stringify(capturedContexts[1].messages);
      expect(reviewPromptText).not.toContain(
        "condition caused to you move to this step",
      );
      expect(reviewPromptText).not.toContain("%CONDITION_JUMP_EXPLANATION%");
    }, 15000);
    ...
    it("jumps back when todos incomplete, advances when complete", async () => {
      ...
      const condResults = test.events.customMessages(
  ...
  describe("incomplete-todos-remaining", () => {
    ...
    it("unchecked items → condition true → jumps back", async () => {
      ...
      writeTodo(test.cwd, "pending.md", [{ text: "Task A", checked: false }]);
  ...
  describe("negative existence conditions", () => {
    it("memory_key_not_exists jumps when the key is absent and advances once the key exists", async () => {
      ...
      test.sendUserMessage(
        "/workflow memory-key-not-exists-branch Build the plan",
      );
      ...
      const explanations = condResults.map((m: any) =>
        String(m.details.explanation),
      );
      expect(explanations).toContain('Memory key "plan" does not exist');
      expect(explanations).toContain('Memory key "plan" exists');
    }, 15000);

    it("file-not-exists resolves filepath from memory and advances once the file exists", async () => {
      test = await createComponentTest();
      registerMockModel(test);
      ...
      test.sendUserMessage(
        "/workflow file-not-exists-memory-branch Build the design",
      );
  ...
  describe("modules", () => {
    ...
    it("merges workflow-level and step-level modules without duplicates", () => {
      ...
      const step = {
        ...
        model: "m",
        prompt: "p",
        modules: ["dev", "agent-todo"],
        maxExecutions: 10,
      } as any;
      const effective = computeEffectiveModules(config, step);
      expect(effective).toEqual(
        expect.arrayContaining(["memory", "dev", "agent-todo"]),
      );
  ...
  describe("model selection", () => {
    it("uses the specified model for each step", async () => {
      ...
      writeWorkflow(test.cwd, "model-test", {
        name: "model-test",
        steps: [{ name: "Step1", model: "mock-model", prompt: "Do work" }],
      });
  ...
  describe("maxExecutions", () => {
    it("stops jumping when maxExecutions reached and advances sequentially", async () => {
        ...
        steps: [
          {
            ...
            conditions: [
              {
                command: "incomplete-todos-remaining",
                args: { todoFilepath: "never-done.md" },
                jump: "Loop",
              },
            ],
          },
      ...
      const markers = test.events.customMessages("workflow:step-marker");
      const loopMarkers = markers.filter(
        (m: any) => m.details.stepName === "Loop",
      );
      expect(loopMarkers).toHaveLength(2);

      const finalMarkers = markers.filter(
        (m: any) => m.details.stepName === "Final",
      );
  ...
  describe("error halt", () => {
    it("pauses on agent error and resumes with /workflow continue", async () => {
      ...
      const markersAfterError = test.events.customMessages(
    ...
    it("handles repeated errors — re-pauses on second error, succeeds on third attempt", async () => {
      ...
      const markers = test.events.customMessages("workflow:step-marker");
      const step1Markers = markers.filter(
        (m: any) => m.details.stepName === "Step1",
      );
      expect(step1Markers).toHaveLength(3); // error, error, success

      const step2Markers = markers.filter(
        (m: any) => m.details.stepName === "Step2",
      );
    ...
    it("error on a step with conditions halts before condition evaluation", async () => {
        ...
        steps: [
          {
            ...
            conditions: [
              {
                command: "incomplete-todos-remaining",
                args: { todoFilepath: "cond-error.md" },
                jump: "Work",
              },
            ],
          },
      ...
      writeTodo(test.cwd, "cond-error.md", [{ text: "Task A", checked: true }]);
      ...
      const condResultsAfter = test.events.customMessages(
        "workflow:condition-result",
      );
  ...
  describe("prompt file injection", () => {
    it("resolves @path prompt from file content", async () => {
      test = await createComponentTest({
        initialPrompts: [
          { name: "test-prompt", content: "INJECTED_PROMPT_CONTENT_XYZ" },
        ],
      });
      registerMockModel(test);
      ...
      writeWorkflow(test.cwd, "prompt-file", {
        name: "prompt-file",
        steps: [{ name: "Step1", model: "mock-model", prompt: "@test-prompt" }],
      });
    ...
    it("resolves nested local prompt directories via /workflow command", async () => {
      test = await createComponentTest({
        initialPrompts: [
          { name: "pack/task", content: "NESTED_LOCAL_PROMPT_CONTENT" },
        ],
      });
      registerMockModel(test);
      ...
      const allNotifications = test.notifications
        .map((n) => n.message)
        .join("\n");
      expect(allNotifications).not.toMatch(
        /prompt reference not found|ambiguous prompt reference/i,
      );
    ...
    it("falls back to ~/.pi/prompts nested directories via /workflow command", async () => {
      test = await createComponentTest({
        initialHomePrompts: [
          { name: "pack/task", content: "HOME_NESTED_PROMPT_CONTENT" },
        ],
      });
      registerMockModel(test);
    ...
    it("shows actionable error for ambiguous nested prompt refs", async () => {
      test = await createComponentTest({
        initialPrompts: [
          { name: "pack/task", content: "LOCAL_PROMPT_CONTENT" },
        ],
        initialHomePrompts: [
          { name: "pack/task", content: "HOME_PROMPT_CONTENT" },
        ],
      });
      registerMockModel(test);
  ...
  describe("context clearing on loop-back jumps", () => {
    it("clears context window when condition loops back to a previous step", async () => {
        ...
        steps: [
          ...
          },
          {
            ...
            conditions: [
              {
                command: "incomplete-todos-remaining",
                args: { todoFilepath: "loop-tasks.md" },
                jump: "Execute",
              },
            ],
          },
      ...
      const markers = test.events.customMessages("workflow:step-marker");
      const executeMarkers = markers.filter(
        (m: any) => m.details.stepName === "Execute",
      );
      expect(executeMarkers).toHaveLength(4);

      // Now verify context isolation for each Execute iteration
      const executeContexts = capturedContexts.filter(
        (c) => c.stepName === "Execute",
      );
      ...
      const summarizeContexts = capturedContexts.filter(
        (c) => c.stepName === "Summarize",
      );
      ...
      expect(test.notifications).toContainEqual(
        expect.objectContaining({
          message: expect.stringMatching(
            /status-test.*Step 1\/2.*FirstStep.*awaiting approval/,
          ),
        }),
      );
    }, 15000);
  ...
  describe("production workflows (end-to-end)", () => {
    it("feature.yml — runs through all steps with approval gate and memory-based condition", async () => {
      ...
      await test.mockAgentResponse({
        text: "I've written the design document.",
      });
        ...
        toolCalls: [
          {
            name: "write",
            args: {
              path: "plans/feature/overview.md",
          ...
          },
          {
            name: "write",
            args: {
              path: "plans/feature/todo.md",
              content: "- [x] Task 1\n- [x] Task 2",
            },
          },
          {
            name: "memory_add",
            args: { store: workflowId, key: "plan", value: "plans/feature" },
          },
          {
            name: "memory_add",
            args: {
      ...
      // --- Step 3: Implement (condition: incomplete-todos-remaining memoryKey=plan-todo) ---
      ...
      const errorNotifications = test.notifications.filter((n) =>
        /prompt reference not found|ambiguous prompt reference/i.test(
          n.message,
        ),
      );
        ...
        steps: [
          {
            name: "Clarify",
            model: "mock-model",
            prompt: "Clarify the research question",
            approval: true,
          },
          {
            name: "Search",
            model: "mock-model",
            prompt: "Search for relevant sources",
          },
          {
            name: "Deep Dive",
            model: "mock-model",
            prompt: "Deep dive into the topic",
          },
          {
            name: "Synthesize",
            model: "mock-model",
            ...
            conditions: [
              {
                prompt: "Is more research needed?",
                model: "mock-model",
                jump: "Search",
              },
            ],
