...
describe("file_not_exists condition command", () => {
	const stores: string[] = [];
	...
	it("is registered as a condition command", () => {
		const fn = getConditionCommand("file_not_exists");
	...
	it("returns true when a literal filepath does not exist", async () => {
		...
		expect(readKey(cwd, store, "workflow-condition-result")).toBe(
			JSON.stringify({ result: "true", explanation: "File does not exist: plans/missing/design.md" }),
		);
	});
	...
	it("returns false when a literal filepath exists as a file", async () => {
		...
		expect(readKey(cwd, store, "workflow-condition-result")).toBe(
			JSON.stringify({ result: "false", explanation: "File exists: plans/existing/design.md" }),
		);
	});
	...
	it("returns true when a literal filepath resolves to a directory", async () => {
		...
		expect(readKey(cwd, store, "workflow-condition-result")).toBe(
			JSON.stringify({ result: "true", explanation: "Path exists but is not a file: plans/directory-target" }),
		);
	});
	...
	it("returns true when memoryKey points to a missing file", async () => {
		...
		expect(readKey(cwd, store, "workflow-condition-result")).toBe(
			JSON.stringify({ result: "true", explanation: "File does not exist: plans/from-memory/design.md" }),
		);
	});
	...
	it("returns false when memoryKey points to an existing file", async () => {
		...
		pathsToClean.push("plans/from-memory/existing.md", "plans/from-memory", "plans");

		await fn(makeCommandCtx(store), { memoryKey: "design-doc" });

		expect(readKey(cwd, store, "workflow-condition-result")).toBe(
			JSON.stringify({ result: "false", explanation: "File exists: plans/from-memory/existing.md" }),
		);
	});
	...
	it("returns true when memoryKey points to a directory", async () => {
		...
		expect(readKey(cwd, store, "workflow-condition-result")).toBe(
			JSON.stringify({ result: "true", explanation: "Path exists but is not a file: plans/from-memory-dir" }),
		);
	});
	...
	it("throws when both memoryKey and filepath are provided", async () => {
		...
		await expect(
			fn(makeCommandCtx(store), { memoryKey: "design-doc", filepath: "plans/design.md" }),
		).rejects.toThrow("file_not_exists: 'memoryKey' and 'filepath' are mutually exclusive");
	});
	...
	it("throws when neither memoryKey nor filepath is provided", async () => {
		...
		await expect(fn(makeCommandCtx(store), {})).rejects.toThrow(
			"file_not_exists requires either 'memoryKey' or 'filepath' arg",
		);
	});
});
