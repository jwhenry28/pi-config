...
import "./commands/incomplete-todos-remaining.js";
...
async function handlePendingConditionContinue(
  ...
  if (!conditionResult) {
    await handleWorkflowAbort(
      pi,
      state,
      ctx,
      "aborted: condition evaluation failed",
      "error",
    );
    return;
  }
  ...
  if (isMaxExecutionsReached(state, conditionResult.jump)) {
    ...
    const targetStep = state.active!.config.steps.find(
      (step) => step.name === conditionResult.jump,
    )!;
    ctx.ui.notify(
  ...
  const previousStepName = currentStep(state)!.name;
  const jumped = jumpToStep(state, conditionResult.jump);
  setConditionJumpContext(state, jumped, previousStepName, conditionResult.explanation);
...
function handleWorkflowStatus(
  ...
  const approvalSuffix =
    isPromptStep(step) && step.approval ? " (awaiting approval)" : "";
  ctx.ui.notify(
    `${state.active.config.name} — Step ${stepIndex}/${totalSteps}: ${step.name}${approvalSuffix}`,
    "info",
  );
}
...
async function handleWorkflowStart(
  ...
  if (state.active) {
    ctx.ui.notify(
      `Workflow "${state.active.config.name}" is already running. Use /workflow abort first.`,
      "warning",
    );
    return;
  }
  ...
  ctx.ui.notify(
    `Starting workflow "${config.name}" (${config.steps.length} steps)`,
    "info",
  );
  await runCurrentStep(pi, state, ctx);
}

function parseWorkflowStartArgs(trimmedArgs: string): {
  workflowName: string;
  userPrompt: string;
} {
  const firstWhitespaceMatch = trimmedArgs.match(/\s/);
  const firstWhitespaceIndex = firstWhitespaceMatch
    ? firstWhitespaceMatch.index!
    : -1;
  const workflowName =
    firstWhitespaceIndex === -1
      ? trimmedArgs
      : trimmedArgs.slice(0, firstWhitespaceIndex);
  const userPrompt =
    firstWhitespaceIndex === -1
      ? ""
      : trimmedArgs.slice(firstWhitespaceIndex + 1).trim();
  return { workflowName, userPrompt };
}

function getModuleState(pi: ExtensionAPI): {
...
function registerEvaluateConditionCommand(
  pi: ExtensionAPI,
  state: WorkflowState,
): void {
  pi.registerCommand("evaluate-condition", {
    description:
      "Manually evaluate a workflow condition: /evaluate-condition <true/false> <explanation>",
    handler: async (args, ctx) => {
...
function handleEvaluateConditionCommand(
  state: WorkflowState,
  args: string,
  ctx: ExtensionCommandContext,
): void {
  ...
  const result =
    firstSpaceIndex === -1
      ? trimmedArgs
      : trimmedArgs.slice(0, firstSpaceIndex);
  const explanation =
    firstSpaceIndex === -1
      ? "Manual evaluation"
  ...
  if (result !== "true" && result !== "false") {
    ctx.ui.notify(
      "Usage: /evaluate-condition <true/false> <explanation>",
      "warning",
    );
    return;
  }
  ...
  ctx.ui.notify(
    `Condition manually evaluated: ${result} — ${explanation}`,
    "info",
  );
