...
export async function executePythonTool(entrypoint: string, input: unknown): Promise<string> {
  const jsonArg = JSON.stringify(input ?? {});

  try {
    const { stdout, stderr } = await execFileAsync("python3", [entrypoint, jsonArg]);
