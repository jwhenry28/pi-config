...
export default function configExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("config", {
    ...
    handler: async (args, ctx) => handleConfig(args, { ...ctx, cwd: getCwd(ctx), storeName: CONFIG_STORE }),
  });
}
  ...
  if (!subcommand || subcommand === "help") {
    ctx.ui.notify(
      [
        ...
        "  apply <name>         Apply a config file",
...
function listConfigFiles(cwd: string): string[] {
  ...
  // Plugin configs
  for (const pluginName of getEnabledPlugins(cwd)) {
    const pluginConfigDir = join(getPluginsDir(), pluginName, "configs");
    if (!existsSync(pluginConfigDir)) continue;
    const files = readdirSync(pluginConfigDir).filter((f) => YAML_EXT_RE.test(f));
    for (const f of files) {
      names.push(`${pluginName}:${f.replace(YAML_EXT_RE, "")}`);
    }
  }
...
function readConfigYaml(cwd: string, name: string): string {
  ...
  if (colonIdx !== -1) {
    // Plugin config: "repo-name:config-name"
    const repo = name.slice(0, colonIdx);
    const configName = name.slice(colonIdx + 1);
    const pluginConfigDir = join(getPluginsDir(), repo, "configs");
    if (!existsSync(pluginConfigDir)) throw new Error(`Plugin config directory not found: ${repo}/configs`);
    const files = readdirSync(pluginConfigDir).filter((f) => YAML_EXT_RE.test(f));
    const match = files.find((f) => f.replace(YAML_EXT_RE, "") === configName);
    if (!match) throw new Error(`Config file not found: ${name}`);
    return readFileSync(join(pluginConfigDir, match), "utf-8");
  }
